...
- `TokenUsage` — 모든 OpenAI 호출의 토큰 소비 로그. 대시보드 집계에 사용. Phase 8-2 부터 `purpose` (CharField + db_index) 가 호출 목적을 분류 — `single_shot_answer / query_rewriter / workflow_extractor / workflow_table_lookup / agent_step / agent_final / unknown` (코드 상수 `chat.services.token_purpose`). `validate_purpose` 가 record 진입 시 알 수 없는 값을 'unknown' 으로 절감해 호출부 오타가 데이터 오염으로 직결되지 않게. Phase 8-5 부터 `cost_usd` (DecimalField) 가 record 시점에 모델 단가 매핑 (`chat.services.openai_pricing.MODEL_PRICING`) × 토큰으로 자동 계산 — 시점 cost 보존. 본 cost 는 **TokenUsage 에 기록된 채팅 LLM 호출 기준 자체 추정** (embedding · reranker 미포함; Phase 8-7 후보). Phase 4-4 OpenAI Admin API widget 의 외부 청구 cost 와 의도적 분리.
...
- `workflows/domains/` — 실제 generic workflow 들과 디스패처 (Phase 6-1 / 6-2 / 6-3). `registry.WorkflowEntry` 는 `{key, title, description, status, factory, input_schema}` 를 들고 있고, `dispatch.run(key, input)` 이 Phase 5 러너로 실행한다. 현재 구현: `general/date_calculation` (두 날짜 사이 기간) · `general/amount_calculation` (숫자/금액 목록의 합계·평균·차이) · `general/table_lookup` (업로드된 문서의 표에서 셀 값 찾기 — 하이브리드 + rerank 검색 결과의 markdown 표를 LLM 이 한 번에 한 셀만 고르게 함). `workflow_key` 가 비었거나 미등록이면 `workflow_node` 가 single_shot 으로 폴백 → 회귀 0. BO `/bo/router-rules/` 의 드롭다운에서 `workflow_key` 를 지정하면 해당 질문이 dispatch 로 흐른다. Phase 6-2 부터 `workflow_input_extractor` 가 질문/history 에서 `input_schema` 기반으로 필요한 값을 자동 추출 (regex 우선, 부족하면 gpt-4o-mini fallback). Phase 6-3 부터 retrieval 이 필요한 workflow (input_schema 에 `'text'` 필드가 있음) 에 한해 Phase 4-3 `query_rewriter` 를 함께 돌려 "그 표에서..." 류 후속 질문도 자립 검색어로 변환된다. 실패/자료 없음/일시 오류는 `WorkflowStatus` 네 값(`NOT_FOUND` / `INVALID_INPUT` / `UPSTREAM_ERROR` / `UNSUPPORTED`) 으로 구분된다.
- `agent/` — generic agent runtime (Phase 7-1) + graph 결선 (Phase 7-2) + snippet windowing (Phase 7-3) + decision policy / retrieval relevance (Phase 7-4) + AgentResult / sources surface (Phase 8-1) + BO 운영 제어 (Phase 8-3 — `AgentSettings` singleton + `runtime_settings.load_runtime_settings()` per-call inject + `enabled=False` 시 single_shot 폴백). ReAct loop 가 등록된 도구(`retrieve_documents` / `find_canonical_qa` / `run_workflow`) 를 반복 호출하며 multi-hop 탐색형 질문을 처리. 진입점은 `chat.services.agent.react.run_agent(question, history) -> AgentResult` (Phase 8-1 — 7-1 의 WorkflowResult 에서 변경). `AgentResult` 는 `status / value / details / termination / tool_calls / sources` 를 1급 필드로 담는다 (`BaseResult` Protocol 매치 — `WorkflowResult` 와 공통 표면). 사용자-facing reply 변환은 `chat.services.agent.reply.build_reply_from_agent_result(result: BaseResult)`. `Tool.input_schema` 는 `Optional[Mapping[str, FieldSpec]]` 로 schema 모드 / raw 모드 두 갈래 (`run_workflow` 는 raw — workflow_key 마다 input 형태가 달라서). 종료 정책은 `max_iterations=6` (7-2 smoke 가 4 부족을 잡아내 상향), 연속 실패 한도(3), 동일 호출 반복 한도(3), **`MAX_LOW_RELEVANCE_RETRIEVES=3` 누적 가드 (7-4 신규)**. Phase 8-1 부터 `AgentResult` 가 별도 dataclass — `WorkflowResult` 호환은 `to_workflow_result()` 어댑터로 유지. `AgentTermination` 내부 사유는 `to_agent_result(termination, *, value, reason, state)` / `to_workflow_result(...)` 가 `OK / NOT_FOUND / UPSTREAM_ERROR` 중 하나로 매핑한다 (`UNSUPPORTED` 는 라우팅 단의 책임이라 agent 가 만들지 않음; reply 단도 ValueError 로 봉쇄). sources 는 `_retrieve_callable` 의 `hits[0]` top-1 evidence → `Observation.evidence` → `to_agent_result` 의 dedup + `low_relevance` 제외 → `agent_node` 가 status 무관 `result.sources_as_dicts()` 로 노출. Phase 7-2 부터 `chat/graph/nodes/agent.py` 가 `ROUTE_AGENT` 진입점 — history-aware rewrite → `run_agent` → reply → `QueryResult`. graph 결선 자동 회귀 테스트(`test_graph_agent_wiring.py`) 가 ROUTE_* 세 분기를 `_compiled_graph.cache_clear()` + import-binding patch 로 검증한다. Phase 7-3 부터 `_summarize_retrieve` 가 query 키워드 매치 위치 주변 forward-bias 윈도우 (top 3 청크 × ±400자, 총 ~1500자 cap). Phase 7-4 부터 (a) `_has_meaningful_match` (longest non-low-signal token tier) 미매치 청크에 `[관련성 낮음]` 마커 + 모든 hit 미매치 시 머리 `[query 핵심 토큰 매치 없음]` 마커, (b) `build_messages` 가 직전 5개 tool 호출 노출 + No call repetition 가이드, (c) runtime guard 3종 — 동일 (tool, args) 호출 차단 (`failure_kind='repeated_call'`) + `Tool.failure_check` 로 retrieve 의 0-hit/all-low-relevance 를 `failure_kind='low_relevance'` 로 마킹 + `MAX_LOW_RELEVANCE_RETRIEVES=3` 누적 가드를 `_decide_termination` 의 max_iter 보다 먼저 평가 → **low_relevance failure 가 3회 이상 누적되면 NOT_FOUND 가 max_iter UPSTREAM_ERROR 보다 우선**해 정직하게 종료. (모든 step 이 false positive 인 진짜 LLM 결함 케이스는 그대로 UPSTREAM_ERROR.)
...
|2026.04.24| 2.0.0 Phase 6-1 — Workflow Dispatch 인프라<br>`chat/workflows/domains/{registry, dispatch, reply}` + `general/date_calculation` + `workflow_key` 를 RouteDecision/GraphState/RouterRule 에 확장 · BO RouterRule 폼에 드롭다운 추가 · graph `workflow_node` 가 dispatch 호출 또는 single_shot 폴백 |[2026-04-24-phase6-1-workflow-dispatch.md](resources/documents/2026-04-24-phase6-1-workflow-dispatch.md)|
|2026.04.24| 2.0.0 Phase 6-2 — Workflow Input Extraction + amount_calculation<br>`FieldSpec` + `workflow_input_extractor` (regex 우선 + LLM fallback) · `general/amount_calculation` 추가 · `workflow_node` 가 자연어 질문에서 자동 추출 → dispatch |[2026-04-24-phase6-2-workflow-input-extraction.md](resources/documents/2026-04-24-phase6-2-workflow-input-extraction.md)|
|2026.04.24| 2.0.0 Phase 6-3 — table_lookup + 남은 정책 다듬기 (Phase 6 완료)<br>`general/table_lookup` (하이브리드 + rerank retrieval → markdown 표 → LLM 셀 선택) · `FieldSpec` 에 `'text'` 타입 + 질문 원문 fallback · `WorkflowStatus` 에 `NOT_FOUND` / `UPSTREAM_ERROR` 추가 · reply 세 status 별 분기 · Phase 4-3 query_rewriter 를 retrieval 필요 workflow 에 재사용 |[2026-04-24-phase6-3-table-lookup.md](resources/documents/2026-04-24-phase6-3-table-lookup.md)|
|2026.04.25| 2.0.0 Phase 7-1 — Agent Runtime (graph 미연결)<br>`chat/services/agent/{state, tools, tools_builtin, prompts, react, result}` + ReAct loop + `retrieve_documents` / `find_canonical_qa` / `run_workflow` 세 도구. graph wiring · reply 포맷터 · BO smoke 는 Phase 7-2. |[2026-04-25-phase7-1-agent-runtime.md](resources/documents/2026-04-25-phase7-1-agent-runtime.md)|
|2026.04.27| 2.0.0 Phase 7-2 — Agent Graph Wiring<br>`chat/graph/nodes/agent.py` + `chat/services/agent/reply.py` + `app.py` ROUTE_AGENT → agent_node 결선 + `test_graph_agent_wiring.py` (graph 단 자동 회귀 테스트 첫 사례). 7-1 튜닝 (`max_iterations` 6, observation cap 1500, retrieve summary top 3×400자, 결정성 프롬프트, iteration INFO 로그) 은 7-2 smoke 가 잡아냄 — query-focused snippet windowing 은 Phase 7-3 으로 분리. |[2026-04-27-phase7-2-agent-graph-wiring.md](resources/documents/2026-04-27-phase7-2-agent-graph-wiring.md)|
|2026.04.27| 2.0.0 Phase 7-3 — Agent Retrieval Snippet Windowing<br>`_tokenize_query` (punctuation strip + len≥2 + len desc sort) + `_focus_window` (forward-bias) helper + `_retrieve_callable` 의 dict wrapping 으로 Tool API 안 깨고 query 를 `_summarize_retrieve` 까지 흘림. 7-2 의 "첫 N자 truncation" 한계 — 답이 401자+ 위치에 있으면 LLM 이 못 보던 회귀 — 를 근본 해결. byte-identical 회귀 0 은 매치가 매우 앞 (`< length//4`) / 미매치 두 케이스에서 보장. |[2026-04-27-phase7-3-agent-snippet-windowing.md](resources/documents/2026-04-27-phase7-3-agent-snippet-windowing.md)|
|2026.04.27| 2.0.0 Phase 7-4 — Agent Decision Policy & Retrieval Relevance (Phase 7 완료)<br>`_has_meaningful_match` (longest non-low-signal token tier) 로 retrieve summary 에 `[관련성 낮음]` / 머리 `[query 핵심 토큰 매치 없음]` 마커 + `build_messages` last K=5 tool calls 노출 + runtime guard 3종 (동일 args 차단 / `Tool.failure_check` 로 0-hit·all-low-relevance failure 마킹 / `MAX_LOW_RELEVANCE_RETRIEVES=3` 누적 가드를 max_iter 보다 우선). 7-3 smoke 의 두 결함 (관련 없는 query 무한 retrieve / 같은 args 핑퐁) 을 LLM 협조 없이 알고리즘적으로 차단. low_relevance 3회+ 누적 시 UPSTREAM_ERROR 미도달 보장. |[2026-04-27-phase7-4-agent-decision-policy.md](resources/documents/2026-04-27-phase7-4-agent-decision-policy.md)|
|2026.04.25| 2.0.0 Phase 8-1 — AgentResult / Reply Surface<br>`BaseResult` Protocol (runtime_checkable) + `AgentResult` (status / value / details / termination / tool_calls / sources) + `SourceRef` / `ToolCallTrace` 1급 필드. `Observation` 에 `arguments` / `evidence` 추가, `tools.call` 의 5 Observation 경로 + react.py 의 3 직접 add_observation 경로가 모두 args 보존. `_retrieve_callable` 이 `hits[0]` 한 건만 evidence 부착 (top-1 정책). `agent_node` 의 `QueryResult.sources = result.sources_as_dicts()` 로 status 무관 노출 (NOT_FOUND 종료여도 그동안 검색된 sources 노출 → 후속 질문 단서). `build_reply_from_agent_result` 시그니처 `BaseResult` 로 좁혀 AgentResult / WorkflowResult 모두 받음. |[2026-04-25-phase8-1-agent-result-reply-surface.md](resources/documents/2026-04-25-phase8-1-agent-result-reply-surface.md)|
|2026.04.28| 2.0.0 Phase 8-2 — TokenUsage Purpose / Agent Observability 기초<br>`TokenUsage.purpose` CharField + db_index 추가 (default 'unknown'). `chat/services/token_purpose.py` 신규 — 6 known 상수 (`single_shot_answer / query_rewriter / workflow_extractor / workflow_table_lookup / agent_step / agent_final`) + `PURPOSE_UNKNOWN` + `ALL_PURPOSES` + `validate_purpose` 방어망. `record_token_usage(model, usage, *, purpose=...)` keyword-only widening — positional 시그니처 보존으로 회귀 0. 7 호출 사이트 (single_shot pipeline 2 / workflow node 2 / agent node 1 / table_lookup 1 / agent.react 1) 가 명시 purpose 전달. agent.react 는 record 위치를 `_parse_action` 직후로 이동해 `action == 'final_answer'` 면 `agent_final`, 아니면 `agent_step` 으로 분기. workflow rewriter 는 `_schema_needs_retrieval` 게이트 그대로 (text 필드 있는 workflow 만 호출). 운영자 표면 — 사용자-facing 변화 없음. |[2026-04-28-phase8-2-token-usage-purpose.md](resources/documents/2026-04-28-phase8-2-token-usage-purpose.md)|
|2026.04.28| 2.0.0 Phase 8-3 — BO Agent 운영 제어<br>`AgentSettings` hard singleton (`save()` override 가 pk=1 강제 + DB CheckConstraints 두 개 — `max_iterations` 1~12 / `max_low_relevance_retrieves` 1~10). `chat/services/agent/runtime_settings.py` 신규 — DEFAULT_* single source of truth + `load_runtime_settings()` per-call DB SELECT + 3단 폴백. `react.py` 가 settings 주입 받아 ReAct loop 한도 적용 (DEFAULT_MAX_ITERATIONS / MAX_LOW_RELEVANCE_RETRIEVES 는 alias 보존). `agent_node` 가 `enabled=False` 시 `single_shot_node(state)` 폴백 (incident 대응 kill switch). BO `/bo/agent/` 페이지 — settings 폼 + tool catalog (읽기 전용) + 최근 7일 호출 통계 (Phase 8-2 purpose 산물 활용). DB CheckConstraint + Form validator + runtime sanity 삼중 가드. 변경 즉시 반영 (캐시 없음). |[2026-04-28-phase8-3-bo-agent-operations.md](resources/documents/2026-04-28-phase8-3-bo-agent-operations.md)|
|2026.04.28| 2.0.0 Phase 8-4 — Polish: Retrieval Relevance & BO Shared Partials<br>**8-1 backlog 처리** — `_LOW_SIGNAL_TOKENS` 에 시간 단어 13개 추가 + `_PARTICLE_SUFFIX` (한국어 조사 그룹) + `_LOW_SIGNAL_PATTERNS` regex tuple (10개) + `_is_low_signal` helper. `2025년` / `1일부터` / `날짜는` 같은 토큰이 longest meaningful tier 에 들어가지 못하도록 차단 — 시나리오 A''' 의 false positive (무관 PDF 가 sources 에 노출) 알고리즘 차원에서 해소. **8-3 carry-over 처리** — `bo/static/bo/bo.js` 신규 (`initFormShake` / `initAlertAutoDismiss` / `initBulkActions`) + `data-bulk-*` 마크업 규약. `data-bulk-edit-toggle data-bulk-target` 짝 매칭으로 토글이 page 컨테이너 밖 header 에 위치해도 동작. `alert-danger` 는 자동 제거 제외. `_qa_bulk_script.html` partial 삭제 + qa / router 페이지 마크업 통일. |[2026-04-28-phase8-4-polish-retrieval-bo-partials.md](resources/documents/2026-04-28-phase8-4-polish-retrieval-bo-partials.md)|
|2026.04.29| 2.0.0 Phase 8-5 — BO Observability: Purpose Breakdown & Cost<br>`TokenUsage.cost_usd` DecimalField 추가 + `chat/services/openai_pricing.py` 신규 (MODEL_PRICING + compute_cost_usd). record_token_usage 가 호출 시점에 모델 단가 × 토큰으로 cost 자동 계산해 row 에 저장 (시점 cost 보존). 미등록 모델은 0 + warning (fail-silent). BO 대시보드 — 상단 카드 5개 (비용 추가) + 일별 표 비용 컬럼 + 신규 `Purpose 별 사용량` 섹션 (observed-only, 한국어 라벨). 비용 caption 에 "TokenUsage 에 기록된 채팅 LLM 호출 기준 자체 추정 (embedding · reranker 미포함)" 명시 — Phase 4-4 OpenAI Admin API widget 의 외부 청구와 의도적 분리. embedding · reranker 통합은 Phase 8-7 후보. |[2026-04-29-phase8-5-bo-observability.md](resources/documents/2026-04-29-phase8-5-bo-observability.md)|
|2026.04.29| 2.0.0 Phase 8-6 — Agent Settings Expansion: Extra Limits & Audit Log<br>`AgentSettings` 에 `max_consecutive_failures` (1~10) / `max_repeated_call` (**2~10**) 두 필드 추가 — `react.py` 의 코드 상수 두 개를 `runtime_settings.DEFAULT_*` alias 로 변경, `_decide_termination` 시그니처 확장. `max_repeated_call=1` 은 첫 정상 tool call 직후 종료 충돌이라 min=2 강제 + help_text 에 호환/기록용 안내 명시 (8-3 즉시 차단 정책으로 실제 동작 변화 거의 없음 — 정책 통합은 후속 Phase). 신규 `AgentSettingsAudit` 모델 — `changed_at / changed_by FK SET_NULL / changes JSON / snapshot JSON`. `agent_view` POST 가 form 바인딩 전 DB 값을 `old_values` 별도 캡처 (ModelForm.is_valid() instance mutation 회피) 후 변경된 필드만 audit row 생성 (변경 0이면 미생성). BO `/bo/agent/` 페이지 하단에 `Settings 변경 이력` 섹션 (최근 10건). |[2026-04-29-phase8-6-agent-extra-limits-audit.md](resources/documents/2026-04-29-phase8-6-agent-extra-limits-audit.md)|
|2026.04.29| 2.0.0 Phase 9 — 안정화 및 배포 준비<br>로드맵 §3 의 본 Phase 8 의도 (안정화 + 배포 준비) 가 실제 개발에선 'agent 운영화' 로 재정의되어, 별 milestone Phase 9 로 분리해 처리. 자동 회귀 가드 — 라우팅 e2e (`test_routing_e2e.py` 9 cases) + 파이프라인 smoke (`test_pipeline_smoke.py` 9 cases, 노드 자체 mock + `_compiled_graph.cache_clear()` 계약) + prompt registry 정합 (`PromptRegistryConsistencyTests` 1 case). 12 대표 질문셋 + 기대 라우트 / 응답 / sources / TokenUsage purpose 분포 정리. `assets/prompts/chat/` 8 파일 사용처 점검 (미사용 0건). `CHANGELOG.md` 신규 — 1.x → 2.0.0 user-facing / operator-facing 변화. 배포 체크리스트 (마이그레이션 0001~0015 / 환경변수 / 빌드 / 데이터 backfill / 롤백 / 모니터링). |[2026-04-29-phase9-stabilization.md](resources/documents/2026-04-29-phase9-stabilization.md)|
