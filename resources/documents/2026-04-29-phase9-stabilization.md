# 2026-04-29 개발 로그 — 0.4.0 Phase 9: 안정화 및 배포 준비
...
- 태그 push (`v0.4.0`) → GitHub Actions → GHCR push → 운영 호스트 SSH pull + `docker compose up -d --build`.
...
총 523/523 그린 (8-6 종료 실측 504 → +19 — 라우팅 e2e 9 + 파이프라인 smoke 9 + prompt registry 1).
...
```
chat
 [X] 0001_vector_extension
 [X] 0002_initial
 ...
 [X] 0015_agentsettingsaudit
...
15 마이그레이션 모두 적용 완료. release 시점에 `migrate` 추가 동작 없음 — 이미 develop 환경에 모두 반영.

### 프롬프트 정합

- `assets/prompts/chat/` 8 파일 사용처 grep — 미사용 0건.
- `prompt_registry.all_entries()` 의 8 entry 의 `relative_path` 모두 파일 존재 — `PromptRegistryConsistencyTests` 통과.
...
- **release tag** — 사용자 명시 요청 시 develop → main PR + `v0.4.0` 태그.
